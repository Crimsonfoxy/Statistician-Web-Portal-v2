...
function add_prefix($db, &$sql, &$values) {
    ...
    if(strpos($sql, DB_PREFIX) !== false) return;
    if(strpos($sql, '%r')) $values[0] = DB_PREFIX . $values[0];
    ...
    elseif (preg_match ("/^INSERT INTO `?prefix_\S+`?\s+[a-z0-9\s,\)\(]*?VALUES/is", $sql)) 
	    $sql = preg_replace("/^INSERT INTO `?prefix_(\S+?)`?([\s\.,]|$)/i", "INSERT INTO `".DB_PREFIX."\\1`\\2", $sql);
